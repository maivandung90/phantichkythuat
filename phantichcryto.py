...
# Hàm lấy dữ liệu
@st.cache_data(ttl=60)  # Cache dữ liệu trong 1 phút
...
def _get_signal_sources(self, signals, df):
    sources = []
    for idx in signals.index:
        ...
        if show_rsi and (df.loc[idx, 'RSI'] < 30 or df.loc[idx, 'RSI'] > 70):
            source.append("RSI")
        if show_macd and (
            (df.loc[idx, 'MACD'] > df.loc[idx, 'MACD_Signal'] and 
             df.shift(1).loc[idx, 'MACD'] <= df.shift(1).loc[idx, 'MACD_Signal']) or
            (df.loc[idx, 'MACD'] < df.loc[idx, 'MACD_Signal'] and 
             df.shift(1).loc[idx, 'MACD'] >= df.shift(1).loc[idx, 'MACD_Signal'])
        ):
            source.append("MACD")
        if show_bollinger and (
            df.loc[idx, 'close'] < df.loc[idx, 'BB_lower'] or 
            df.loc[idx, 'close'] > df.loc[idx, 'BB_upper']
        ):
            source.append("Bollinger")
        if show_ma and (
            (df.loc[idx, f'MA_{ma_fast}'] > df.loc[idx, f'MA_{ma_slow}'] and 
             df.shift(1).loc[idx, f'MA_{ma_fast}'] <= df.shift(1).loc[idx, f'MA_{ma_slow}']) or
            (df.loc[idx, f'MA_{ma_fast}'] < df.loc[idx, f'MA_{ma_slow}'] and 
             df.shift(1).loc[idx, f'MA_{ma_fast}'] >= df.shift(1).loc[idx, f'MA_{ma_slow}'])
        ):
